...
namespace Core.Core.Interfaces.Staff
{
    public interface IStaffIncidentRepository : IGenericRepository<StaffIncident>
    {
        ...
        Task<IEnumerable<StaffIncident>> GetIncidentsBySeverityAsync(Severity severity, CancellationToken cancellationToken = default);
        Task<IEnumerable<StaffIncident>> GetIncidentsByStatusAsync(StaffIncidentStatus status, CancellationToken cancellationToken = default);
