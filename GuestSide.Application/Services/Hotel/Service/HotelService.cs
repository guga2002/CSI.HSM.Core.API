...
namespace Core.Application.Services.Hotel.Service
{
    public class HotelService : GenericService<HotelRequestDto, HotelResponse, long, Core.Entities.Hotel.Hotel>, IHotelService
    {
        ...
        public async Task<bool> UpdateHotelDetails(long hotelId, string? address, string? description, List<string>? pictures, List<string>? facilities, CancellationToken cancellationToken = default)
        {
            ValidatePositiveId(hotelId, nameof(hotelId));
            ...
            ValidateString(address, nameof(address));
            ValidateString(description, nameof(description));

            var result = await _hotelRepository.UpdateHotelDetails(hotelId, address, description, pictures, facilities);
            if (result)
            {
